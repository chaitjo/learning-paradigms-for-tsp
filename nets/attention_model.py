...
from nets.encoders.gcn_encoder import GCNEncoder
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.encoder_class is GCNEncoder:
            self.init_edge_embed = nn.Linear(1, embedding_dim)
    ...
    def forward(self, nodes, supervised_mode=False, targets=None, return_pi=False):
        """
        :param nodes: (batch_size, graph_size, node_dim)
        :param return_pi: whether to return the output sequences, this is optional as it is not compatible with
        using DataParallel as the results may be of different lengths on different GPUs
        :param supervised_mode: Toggles SL mode, teacher forcing and NLL loss computation
        :param targets: Targets for teacher forcing and NLL loss computation in SL mode
        :return:
        """
        
        if self.checkpoint_encoder:
            embeddings = checkpoint(self.embedder, self._init_embed(nodes))['node_embs']
        else:
            embeddings = self.embedder(self._init_embed(nodes))['node_embs']

            
        if self.problem.NAME == 'tspsl' and supervised_mode:
            assert targets is not None, "Pass targets during training in supervised mode"
            
            _log_p, pi = self._inner(nodes, embeddings, supervised_mode=supervised_mode, targets=targets)
            cost, mask = self.problem.get_costs(nodes, pi)
            
            logits = _log_p.permute(0, 2, 1)  # B x V x output_vocab -> B x output_vocab x V
            logits[logits == -float(np.inf)] = -20  # Set -inf values to -20
            ...
            _log_p, pi = self._inner(nodes, embeddings)
            cost, mask = self.problem.get_costs(nodes, pi)
    ...
    def precompute_fixed(self, input):
        embeddings = self.embedder(self._init_embed(input))['node_embs']
    ...
    def _init_embed(self, input):
        ...
        # TSP with GCNEncoder
            ...
            input_temp = input[:, :, None].expand(batch_size, num_nodes, num_nodes, node_dim).reshape((-1, node_dim))
            ...
            edges = nn.PairwiseDistance(p=2, keepdim=False)(
                input_temp, input_temp).reshape((batch_size, num_nodes, num_nodes))
            return self.init_embed(input), self.init_edge_embed(edges.unsqueeze(3))
        
        # TSP with GraphAttentionEncoder/MLPEncoder
        return self.init_embed(input)

    def _inner(self, input, embeddings, supervised_mode=False, targets=None):
        ...
        while not (self.shrink_size is None and state.all_finished()):
            ...
            if self.problem.NAME == 'tspsl' and i == 0:
                # Always start from node 0 for TSP in the supervised setting
                selected = torch.LongTensor([0]*batch_size)
                if self.use_cuda:
                    selected = selected.cuda()
            
            elif self.problem.NAME == 'tspsl' and supervised_mode:
                # Teacher-forcing during training in supervised mode
                t_idx = torch.LongTensor([i])
                if self.use_cuda:
                    t_idx = t_idx.cuda()
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        return sample_many(
            ...
            (input, self.embedder(self._init_embed(input))['node_embs']),  # Pack input with embeddings (additional input)
            batch_rep, iter_rep
        )
